...
(function () {
    ...
    function calculate(v) {
        ...
        // VERTICAL GRID
        var verticalLine = lineSpace;
        var verticalGridline =
            verticalLine * v.verticalGroup;
        var offsetGridVertical =
            offsetSourceValue(v.rowOffsetSourceIndex) / 2;

        // HORIZONTAL GRID
        var horizontalLine = lineSpace;
        var horizontalGridline =
            horizontalLine * v.horizontalGroup;
        var offsetGridHorizontal =
            offsetSourceValue(v.columnOffsetSourceIndex) / 2;

        // GRID MARGIN
        var gridMarginHorizontal = horizontalLine - offsetGridHorizontal;
        var gridMarginVertical = verticalLine - offsetGridVertical;
        ...
        var gridWidth = horizontalGridline * v.gridGroupHorizontal;
        var gridHeight = verticalGridline * gridGroupVertical;

        // MARGIN
        var marginTop = verticalLine * v.marginTopFactor - offsetGridVertical;
        var marginBottom = verticalLine * v.marginBottomFactor - offsetGridVertical;
        var marginLeft = horizontalLine * v.marginLeftFactor - offsetGridHorizontal;
        var marginRight = horizontalLine * v.marginRightFactor - offsetGridHorizontal;

        // FORMAT — exact spreadsheet formulas
        var pageWidth =
            gridWidth +
            (v.marginLeftFactor * horizontalLine) - horizontalLine +
            (v.marginRightFactor * horizontalLine) - horizontalLine;

        var pageHeight =
            gridHeight +
            (v.marginTopFactor * verticalLine) - verticalLine +
            (v.marginBottomFactor * verticalLine) - verticalLine;
        ...
        // INTERSECTION — exact spreadsheet formulas
        var allHorizontalGutters =
            Math.max(0, v.gridGroupHorizontal - 1) * gridGutterHorizontal;
        var allVerticalGutters =
            Math.max(0, gridGroupVertical - 1) * gridGutterVertical;
    ...
    function setupDocumentAndBaselineGrids(
        ...
        // DOCUMENT GRID
        // Each grid square equals Line Space × Line Space.
        try {
            gp.horizontalGridlineDivision = r.lineSpace;
    ...
    var oPageWidth = addRow(pPage, "Width", "", false, "mm");
    var oPageHeight = addRow(pPage, "Height", "", false, "mm");
    ...
    for (var ii = 0; ii < inputFields.length; ii++) {
        (function (field) {
            if (isLinkedTypeSizeField(field)) return;
