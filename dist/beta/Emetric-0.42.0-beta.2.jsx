...
(function () {
    ...
    function addBoundaryGuides(page, layer, r, guideColor, createColumns, createRows, mirrorHorizontal) {
        // Horizontal Grid Guides (major columns) are centered over the Type
        // Area, not the page. This keeps the guide structure visually
        // centered when left and right margins use different values.
        var startX = r.marginLeft + ((r.typeAreaWidth - r.gridWidth) / 2);

        // Vertical Grid Guides are a page-anchored reference grid, not a
        // margin/type-area guide: they measure from the document format's
        // physical top edge, not from the margin. The first guide sits one
        // full Grid Module Height (Steps x Grid Interval) down from the
        // page edge, with each following guide one more module down —
        // there is no guide at the page edge itself (row 0).
        //
        // setupDocumentAndBaselineGrids() moves the document's vertical
        // zero point one Offset Grid above the page edge (for the native
        // document grid), and that shifted ruler is still in effect here.
        // Guide locations are placed in that same coordinate space, so the
        // Offset has to be added back or every guide lands one Offset Grid
        // higher on the page than intended.
        var startY = r.verticalGridline + r.offsetGridVertical;
    ...
    function presetIconColor(
        ...
        if (!active) {
            if (!background) {
                // The preset icon control is a plain "statictext" repurposed
                // for custom drawing (see addPresetIconControl()), so it has
                // no fill of its own for graphics.backgroundColor to report
                // -- this fallback is used for every theme, not just an edge
                // case. Reuse presetHoverPalette()'s per-theme measured
                // Adobe tones (four distinct values) instead of collapsing
                // Dark/Medium Dark and Medium Light/Light into one guess
                // each: that previous two-value approximation made the
                // disabled icon nearly invisible in Medium Dark, whose real
                // panel tone is lighter than Dark's.
                background = presetHoverPalette().fill;
            }
    ...
    function drawSavePresetPictogram(
        ...
        function corner(cp1x, cp1y, cp2x, cp2y, x, y) {
            if (canCurve) {
                graphics.curveTo(
                    px(cp1x), py(cp1y),
                    px(cp2x), py(cp2y),
                ...
                // A rounded corner this small reads fine as a sharp one.
                graphics.lineTo(px(x), py(y));
            }
        }

        // Traced from the Adobe-style 16 x 16 "Save" pictogram (Figma
        // export, see docs/maintenance-notes.md). Drawn as ONE simple
        // path: the indicator notch is cut with a keyhole bridge -- a
        // line out to the notch and back along the same coordinates --
        // instead of a second subpath, because ScriptUI's Graphics path
        // here does not reliably support starting a further subpath (a
        // second moveTo()) within one newPath()/fillPath() pair; that
        // silently produced no icon at all rather than a hole.
        graphics.newPath();
        graphics.moveTo(px(15), py(7));
        corner(15.5, 7, 16, 7.5, 16, 8);
        graphics.lineTo(px(16), py(11));
        corner(16, 11.5, 15.5, 12, 15, 12);
        graphics.lineTo(px(1), py(12));
        corner(0.5, 12, 0, 11.5, 0, 11);
        graphics.lineTo(px(0), py(8));

        // Keyhole cut to the indicator notch and back.
        graphics.lineTo(px(2), py(8));
        graphics.lineTo(px(2), py(9));
        graphics.lineTo(px(4), py(9));
        graphics.lineTo(px(4), py(8));
        graphics.lineTo(px(2), py(8));
        graphics.lineTo(px(0), py(8));

        corner(0, 7.5, 0.5, 7, 1, 7);
        graphics.lineTo(px(15), py(7));
        graphics.closePath();
        graphics.fillPath(brush);

        // Downward save arrow.
        graphics.newPath();
        graphics.moveTo(px(9), py(3));
        graphics.lineTo(px(11.5), py(3));
        graphics.lineTo(px(8), py(6));
        graphics.lineTo(px(4.5), py(3));
        graphics.lineTo(px(7), py(3));
        graphics.lineTo(px(7), py(0));
        graphics.lineTo(px(9), py(0));
        graphics.lineTo(px(9), py(3));
        graphics.closePath();
        graphics.fillPath(brush);
    }
    ...
    function drawDeletePresetPictogram(
        ...
        // Traced from the Adobe-style 16 x 16 "Delete" pictogram (Figma
        // export, see docs/maintenance-notes.md). An earlier attempt cut
        // the front face, its three ribs and the handle gap as nested
        // "keyhole" bridges within one self-touching path -- verified
        // correct as pure geometry by simulating the exact point sequence
        // outside InDesign, but it rendered as one solid silhouette with
        // no holes at all in real InDesign, and a follow-up rectangles
        // only version lost every rounded corner and the clearance below
        // the ribs, reading as too blocky.
        //
        // This version draws the handle and the bin body as two simple
        // *open* outlines instead: a "staple" for the handle and a "U"
        // for the bin body, each a single closed path with no repeated
        // or coincident points anywhere -- unlike a keyhole bridge, which
        // revisits the same coordinates twice to fake a hole. Both
        // shapes are open on the side where their hollow connects
        // straight through to unpainted space outside the shape (the
        // handle's gap opens down into the lid; the bin's front face
        // opens up into the lid), so there is no enclosed hole to cut
        // and no bridge trick needed, while the outer corners can still
        // be genuinely rounded with curveTo(). The lid and the three
        // ribs stay simple independent rectangles, same as before.
        graphics.newPath();
        graphics.moveTo(px(6), py(2));
        graphics.lineTo(px(6), py(1.5));
        corner(6, 0.75, 6.75, 0, 7.5, 0);
        graphics.lineTo(px(9.5), py(0));
        corner(10.25, 0, 11, 0.75, 11, 1.5);
        graphics.lineTo(px(11), py(2));
        graphics.lineTo(px(10), py(2));
        graphics.lineTo(px(10), py(1));
        graphics.lineTo(px(7), py(1));
        graphics.lineTo(px(7), py(2));
        graphics.closePath();
        graphics.fillPath(brush);
        ...
        // Bin body: side walls and bottom traced as one open "U", with
        // the two outer bottom corners rounded to match Adobe's icon.
        graphics.newPath();
        graphics.moveTo(px(13), py(3));
        graphics.lineTo(px(13), py(12));
        corner(13, 12.5, 12.5, 13, 12, 13);
        graphics.lineTo(px(5), py(13));
        corner(4.5, 13, 4, 12.5, 4, 12);
        graphics.lineTo(px(4), py(3));
        graphics.lineTo(px(5), py(3));
        graphics.lineTo(px(5), py(12));
        graphics.lineTo(px(12), py(12));
        graphics.lineTo(px(12), py(3));
        graphics.closePath();
        graphics.fillPath(brush);

        // Three vertical ribs, inside the unpainted front face, with
        // clearance below them before the bin's inner bottom edge.
        presetFillRectangle(
            graphics,
            px(6), py(4),
            1 * scale, 7 * scale,
    ...
    function addPresetIconControl(
        ...
        control.onDraw = function () {
            ...
            // The source geometry is already a 16 × 16 icon (traced
            // directly from the Figma/Adobe pictogram export), drawn 1:1.
    ...
    function presetSettingsEqual(a, b) {
        try {
            return (
                presetStableString(
                    presetComparableSettings(a)
                ) ===
                presetStableString(
                    presetComparableSettings(b)
                )
            );
        } catch (_) {
            return false;
    ...
    function presetComparableSettings(settings) {
        // With Metric Source = Selected Font, Ascender/Cap Height/
        // x-Height/Descender are never stable saved values: update()
        // recomputes them live from whichever font is CURRENTLY
        // installed and selected every time it runs (see
        // selectedTypeRatiosForCalculation() / activeTypeRatios), not
        // from a fixed number tied to the preset. If the preset's saved
        // font isn't installed, Emetric silently substitutes another
        // one (findPreferredFontFamilyIndex()'s fallback), whose
        // measured metrics naturally differ numerically from the
        // original font's -- which flagged the preset "Modified" purely
        // from that substitution, even with no user edits at all. Font
        // Family/Font Style (the user's actual saved intent) still
        // compare normally below. Other Metric Sources (Decimal,
        // Dozenal, Custom Metric) use fixed ratios or literal typed
        // values rather than live font measurement, so this exclusion
        // does not apply to them.
        if (
            !settings ||
            typeof settings !== "object" ||
            settings.metricSourceKey !==
                "selectedFont" ||
            !settings.typeSize
        ) {
            return settings;
    ...
    function backupUnreadablePresetFile(file) {
        // Preserve the existing file's bytes before Emetric can ever write
        // an empty in-memory store back over it. A momentary read failure
        // (a locked/syncing file, a truncated write from a crash, etc.)
        // must never translate into silent, permanent preset loss.
        try {
            var now = new Date();
            var stamp =
                padNumber(now.getFullYear(), 4) +
    ...
    function handlePresetLoadFailure(file, error) {
        var backupPath =
            backupUnreadablePresetFile(file);

        reportDiagnosticError(
            "Load preset data",
            error,
            "Emetric could not read your saved presets" +
                (
                    backupPath
                        ? (
                ...
                "\rEmetric will start with empty presets " +
                "this time instead of overwriting the " +
                "existing file.",
            true
        );
    }
    ...
    function loadPresetStore() {
        ...
        try {
            ...
            if (!file.open("r")) {
                handlePresetLoadFailure(
                    file,
                    new Error(
                        "The preset file could not be " +
                        "opened for reading."
                    )
                );
            ...
            handlePresetLoadFailure(file, error);
    ...
    function applyPresetSettings(
        ...
        try {
            ...
            // Restored before activateSelectedFontMetrics()/
            // activateEmetricSource() run below: both end by calling
            // update(), which recomputes Ascender/Cap Height/x-Height/
            // Descender from the CURRENT fMetrics text. Restoring
            // fMetrics here first means that recompute already uses the
            // preset's own base size instead of whatever base size was
            // left over from the previously active preset.
            var typeSize =
                settings.typeSize || {};
    ...
    function findPreferredFontFamilyIndex() {
        // Tried in this order; the first one actually installed wins.
        // Falls through to index 0 (whatever is first alphabetically)
        // only if none of these are installed.
        ...
        for (var p = 0; p < preferredFamilies.length; p++) {
            ...
            for (var i = 0; i < installedFontFamilies.length; i++) {
                // "Starts with" rather than an exact match, so e.g.
                // "Times New" also matches an installed "Times New
                // Roman", and "Minion" also matches "Minion Pro" even
                // though "Minion Pro" is itself a later, more specific
                // entry in the list above.
                if (
                    String(installedFontFamilies[i])
                        .toLowerCase()
                        .indexOf(preferred) === 0
                ) {
                    return i;
    ...
    function reset() {
        ...
        // Reset had never re-selected a font at all, so it silently kept
        // whatever family/style the user had picked before pressing
        // Reset -- meaning the very next dirty-check would compare that
        // leftover font against defaultPresetSettings' preferred/
        // fallback font and immediately show "Default -- Modified" with
        // no further action from the user. Reset now always re-resolves
        // the preferred font the same way dialog startup does, so it
        // reliably lands back on Default's own font every time.
        fontSelectionIsUpdating = true;
        try {
            fontFamilyDropdown.selection =
                findPreferredFontFamilyIndex();
        ...
        facingPages.value = false;
        try { typeLedFormat.value = true; } catch (_) {}
        anamorphicFormat.value = false;
        lockFormatRatio.value = false;
        updateAnamorphicFormatControls();
        updateMarginLabels();
    ...
    function getDocumentOptions() {
        return {
            ...
            metricSourceName:
                selectedMetricSourceName(),
            // Custom Metric now manually drives the visible Type Size
            // fields directly (see customTypeSizeValuesFromFields()); the
            // legacy customRatio* fields are a hidden, disconnected group
            // that no longer tracks live edits, so build this from the
            // fields the user actually sees and edits.
            customRatioText:
                fMetrics.text + ":" +
                oAscender.text + ":" +
                oUppercase.text + ":" +
    ...
    typeLedFormat.onClick = function () {
        typeLedFormat.value = true;
        anamorphicFormat.value = false;
        anamorphicFormatChanged();
    };
    anamorphicFormat.onClick = function () {
        anamorphicFormat.value = true;
        typeLedFormat.value = false;
        anamorphicFormatChanged();
    };
    ...
    createButton.onClick = function () {
        ...
        // performPreviewUpdate() disables screen redraw while it rebuilds
        // the live preview document, which is exactly the same
        // layers/guides/styles/text-frame work createDocument() does here
        // for the final document. Without the same toggle, this final
        // build repaints the screen on every step and can feel noticeably
        // slower or less consistent than the live preview it usually
        // follows, especially with Live Preview left on (closing that
        // preview document immediately before is also unavoidably part of
        // this click).
        var oldRedraw = true;
