...
(function () {
    ...
    function createParagraphStyles(doc, r, exportUnitIndex, options) {
        ...
        var placeholder =
            doc.paragraphStyles.itemByName("Placeholder Text");

        if (!validObject(placeholder)) {
            placeholder = doc.paragraphStyles.add({
        ...
        try {
            applySelectedFont(
                placeholder,
                options && options.selectedFontRecord
                    ? options.selectedFontRecord
                    : null
            );

            placeholder.pointSize =
                exportPointValue(r.metrics, exportUnitIndex);
            placeholder.leading =
                exportPointValue(r.lineSpace, exportUnitIndex);
            placeholder.alignToBaseline = true;
            placeholder.justification = Justification.LEFT_ALIGN;
            placeholder.leftIndent = "0 mm";
            placeholder.firstLineIndent = "0 mm";
            placeholder.spaceBefore = "0 mm";
            placeholder.spaceAfter = "0 mm";

            try {
                placeholder.paragraphDirection =
                    ParagraphDirectionOptions.LEFT_TO_RIGHT_DIRECTION;
            } catch (_) {}

            try {
                placeholder.digitsType = DigitsTypeOptions.ARABIC_DIGITS;
            } catch (_) {}
        } catch (_) {}
